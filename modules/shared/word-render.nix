...
let
  ...
  # Windows/Word-compatible Latin Modern.
  #
  # Stock lmodern cannot be used in the guest as-is: Word will not render
  # CFF-flavoured OpenType (it lists the family in Application.FontNames, then
  # silently substitutes Calibri on export), and it matches families on name
  # ID 1, where Latin Modern stores the optical size ("LM Roman 10") rather
  # than the typographic family ("Latin Modern Roman", which lives in ID 16).
  # mk_winfonts.py converts the outlines to glyf and rewrites the name table.
  # Verified end-to-end: the L&E sample renders LMRoman10-{Regular,Bold,Italic}
  # + LatinModernMath-Regular with no Cambria/Calibri fallback.
  latinModernWin = pkgs.runCommand "latin-modern-win-ttf"
    {
      nativeBuildInputs = [ (pkgs.python3.withPackages (ps: [ ps.fonttools ])) ];
    }
    ''
      mkdir -p $out
      python3 ${./word-render/mk_winfonts.py} \
        ${pkgs.lmodern}/share/fonts/opentype/public/lm \
        ${pkgs.lmodern}/share/fonts/opentype/public/lm-math \
        $out
    '';
in
{
  options.programs.wordRender = {
    ...
    compareScript = lib.mkOption {
      type = lib.types.str;
      default = "C:/Users/word/compare_docx.ps1";
      description = ''
        Path to compare_docx.ps1 as dropped inside the guest. Kept separate from
        `guestScript` because both are exported into the shell and a single
        variable would point compare at the renderer.
      '';
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    # The compare pair, deployed the same way. Word's CompareDocuments diffs
    # footnote-internal text, which LibreOffice's does not — on a footnote-heavy
    # manuscript that is the difference between a redline and an empty one.
    home.file.".local/share/word-render/compare_docx.ps1".source =
      ./word-render/compare_docx.ps1;

    home.file.".local/share/word-render/word_compare_remote.sh" = {
      source = ./word-render/word_compare_remote.sh;
      executable = true;
    };
    ...
    # Guest font kit: the built TTFs plus the transport/registration scripts.
    # Without these the guest renders Cambria/Calibri for any Latin Modern
    # document — see the latinModernWin comment above.
    home.file.".local/share/word-render/fonts" = {
      source = latinModernWin;
      recursive = true;
    };
    home.file.".local/share/word-render/vm/install-guest-fonts.sh"  = { source = ./word-render/vm/install-guest-fonts.sh; executable = true; };
    home.file.".local/share/word-render/vm/install-guest-fonts.ps1".source = ./word-render/vm/install-guest-fonts.ps1;
    home.file.".local/share/word-render/mk_winfonts.py".source = ./word-render/mk_winfonts.py;
    ...
    # Launcher on PATH: word-render-install-fonts (one-time per guest, and
    # again whenever the font build changes).
    home.file.".local/bin/word-render-install-fonts" = {
      executable = true;
      text = ''
        #!/usr/bin/env bash
        # Generated by modules/shared/word-render.nix — do not edit.
        export WINVM_SSH="''${WINVM_SSH:-${cfg.sshTarget}}"
        exec "$HOME/.local/share/word-render/vm/install-guest-fonts.sh" "$@"
      '';
    };
    ...
    # Convenience launcher on PATH:  word-compare <base.docx> <rev.docx> <out.docx> [stats.txt]
    home.file.".local/bin/word-compare" = {
      executable = true;
      text = ''
        #!/usr/bin/env bash
        # Generated by modules/shared/word-render.nix — do not edit.
        export WINVM_SSH="''${WINVM_SSH:-${cfg.sshTarget}}"
        export WINVM_DIR="''${WINVM_DIR:-${cfg.guestDir}}"
        export WINVM_COMPARE_SCRIPT="''${WINVM_COMPARE_SCRIPT:-${cfg.compareScript}}"
        exec "$HOME/.local/share/word-render/word_compare_remote.sh" "$@"
      '';
    };
    ...
    home.sessionVariables = {
      ...
      WINVM_COMPARE_SCRIPT = lib.mkDefault cfg.compareScript;
    };
  };
}
