...
let
  ...
  # core's qmake base.pri auto-detects the platform and uses this prefix for
  # all 3dParty staging dirs (CORE_BUILDS_PLATFORM_PREFIX)
  platformDir =
    if !isDarwin then "linux_64"
    else if stdenv.hostPlatform.isAarch64 then "mac_arm64"
    else "mac_64";
  ...
  ''
  + (if isDarwin then ''
    mkdir -p $BUILDRT/Common/3dParty/icu/${platformDir}/build
    for f in ${icu}/lib/*; do
      ln -sf $f $BUILDRT/Common/3dParty/icu/${platformDir}/build/
    done
    ln -sfn ${icu.dev}/include $BUILDRT/Common/3dParty/icu/${platformDir}/build/include
  '' else ''
    mkdir $BUILDRT/Common/3dParty/icu/linux_64
    ln -s ${icu}/lib $BUILDRT/Common/3dParty/icu/linux_64/build
  ''));
  icuQmakeFlags =
    lib.optionals (!isDarwin) [ "QMAKE_LFLAGS+=-Wl,--no-undefined" ]
    ++ [
      "QMAKE_LFLAGS+=-licuuc"
  ...
  # apple_silicon: base.pri only switches CORE_BUILDS_PLATFORM_PREFIX to
  # mac_arm64 (and sets QMAKE_APPLE_DEVICE_ARCHS=arm64) when this CONFIG is
  # set — build_tools sets it externally, so must we.
  qmakeFlags =
    lib.optionals (isDarwin && stdenv.hostPlatform.isAarch64) [ "CONFIG+=apple_silicon" ]
    # several components embed core's freetype, compiled with brotli/WOFF2
    # support but without the bundled brotli sources (a build_tools fetch) —
    # link nixpkgs brotli everywhere on darwin
    ++ lib.optionals isDarwin [
      "QMAKE_LFLAGS+=-lbrotlidec"
      "QMAKE_LFLAGS+=-lbrotlicommon"
    ];
  dontStrip = false;
  ...
  buildCoreComponent =
    ...
    stdenv.mkDerivation (
      finalAttrs:
      {
        ...
        # qmake emits dylibs with bare install_names; absolute-ize them so
        # consumers in other derivations resolve them at runtime
        ++ lib.optionals isDarwin [ fixDarwinDylibNames ];
        ...
        prePatch = ''
          ...
          ${lib.optionalString isDarwin ''
            # core_mac block re-pins c++14 AFTER common-cpp20.patch's global
            # c++20 — and icu 76 headers need >=17
            substituteInPlace $BUILDRT/Common/base.pri \
              --replace-quiet "CONFIG += c++14" "CONFIG += c++20"
          ''}
        '';
        postPatch = ''
          cd $SRCRT
      ...
      // {
        buildInputs =
          (attrs.buildInputs or [ ]) ++ lib.optionals isDarwin [ brotli ];
      }
    );
  buildCoreTests =
    rootdir: attrs:
  ...
  graphics = buildCoreComponent "DesktopEditor/graphics/pro" {
    ...
    preConfigure = ''
      ...
      mkdir -p $BUILDRT/Common/3dParty/heif/libheif/build/${platformDir}/release
      ln -s ${libheif.lib}/lib $BUILDRT/Common/3dParty/heif/libheif/build/${platformDir}/release/libheif
      mkdir -p $BUILDRT/Common/3dParty/heif/x265_git/build/${platformDir}
      ln -s ${x265}/lib $BUILDRT/Common/3dParty/heif/x265_git/build/${platformDir}/release
      mkdir -p $BUILDRT/Common/3dParty/heif/libde265/build/${platformDir}/release
      ln -s ${libde265}/lib $BUILDRT/Common/3dParty/heif/libde265/build/${platformDir}/release/libde265
  ...
  doctrenderer = buildCoreComponent "DesktopEditor/doctrenderer" {
    ...
    qmakeFlags =
      ...
      ++ (
        if isDarwin then [
          # macOS uses Apple JavaScriptCore, not v8 — same as ONLYOFFICE's
          # official mac builds (build_tools never fetches v8 on mac)
    ...
    preConfigure = ''
      ...
      echo "== openssl =="
      mkdir -p Common/3dParty/openssl/build/${platformDir}/lib
      echo "Including openssl from ${openssl'.dev}"
      ln -s ${openssl'.dev}/include Common/3dParty/openssl/build/${platformDir}/include
      for i in ${openssl'.out}/lib/*; do
        ln -s $i Common/3dParty/openssl/build/${platformDir}/lib/$(basename $i)
      done
  ...
  fb2file = buildCoreComponent "Fb2File" {
    ...
    qmakeFlags = qmakeFlags ++ lib.optionals (!isDarwin) [
  ...
  ooxmlsignature = buildCoreComponent "DesktopEditor/xmlsec/src" {
    ...
    preConfigure = ''
      mkdir -p $BUILDRT/Common/3dParty/openssl/build/${platformDir}/lib
      for i in ${openssl'.out}/lib/*; do
        ln -s $i $BUILDRT/Common/3dParty/openssl/build/${platformDir}/lib/$(basename $i)
      done
    '';
