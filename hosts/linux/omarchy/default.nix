...
exit 0
  ...
  aercHtmlChawan = pkgs.writeShellScript "aerc-html-chawan" ''
    ...
    COLS=''${COLUMNS:-80}
    # CAPPED AT 1.0 -- the zoom is OFF, and this is not a leftover.
    # vaxis >= 0.16.0 culls any graphic that does not fit the viewport WHOLE
    # (`viewportRow+img.rows > vt.height() -> continue`, widgets/term/term.go).
    # Scaling the mail up makes every image taller in cells, so more of them
    # fail that test and vanish completely. Measured: at 2.5x the newsletter
    # laid out full-width and rendered NO images at all. Width and images are
    # in direct tension under this vaxis; images win.
    SCALE=1.0
  ...
  claudeHerdrSpawn = pkgs.writeShellScript "claude-herdr-spawn" ''
    ...
    if [ -n "''${WORKSPACE:-}" ]; then
      CREATED=$(herdr tab create --workspace "$WORKSPACE" --cwd "$DIR" --label "$LABEL" --no-focus) \
        || { echo "herdr tab create failed in workspace $WORKSPACE for $DIR" >&2; exit 1; }
  ...
  claudeRoutines = {
    ...
    "claude-morning-briefing" = {
      ...
      script = pkgs.writeShellScript "claude-morning-briefing" ''
        ...
        # NOT `|| true`: the spawn IS this unit's purpose, so its exit status has
        # to become the unit's. Masking it is what let a failed 07:00 briefing
    ...
    "claude-morning-planning" = {
      ...
      script = pkgs.writeShellScript "claude-morning-planning" ''
        ...
        # Exit non-zero: with no session there is nothing to route into and the
        # day's planning is silently lost. `echo` alone would exit 0 and record
        # the miss as a success.
        echo "no live assistant session — skipping planning (briefing likely did not run)" >&2
        exit 75
      '';
    };
        ...
        if command -v herdr >/dev/null 2>&1 && herdr status server >/dev/null 2>&1; then
          ...
          for t in ''${tabs:-}; do
            if herdr tab close "$t" >/dev/null 2>&1; then
              ...
              echo "FAILED to close herdr tab $t" >&2; failed=$((failed + 1))
            fi
          done
        fi
        # Any lingering daemon sessions (adopted, or spawned before the move).
        # `claude` is a mise shim, and mise chatter can reach its STDOUT — which
        # `2>/dev/null` does not catch. Feeding that to jq produced
        # `jq: parse error: Invalid numeric literal at line 1, column 5` on
        # 2026-08-15, so `ids` came out empty and nothing was stopped, silently.
        # Validate first: unparseable means "could not tell", not "none".
        agents_json=$(claude agents --json 2>/dev/null)
        if printf '%s' "$agents_json" | jq -e . >/dev/null 2>&1; then
          ids=$(printf '%s' "$agents_json" \
            | jq -r '.[] | select(.name == "🦞 assistant" or .name == "morning-briefing" or .name == "nightly-wrapup") | .id')
        else
          ids=""
          echo "could not read 'claude agents --json' — not stopping daemon sessions" >&2
          failed=$((failed + 1))
        fi
        for id in ''${ids:-}; do
          [ "$id" = null ] && continue
          if claude stop "$id"; then
            echo "stopped $id"; stopped=$((stopped + 1))
          else
            echo "FAILED to stop $id" >&2; failed=$((failed + 1))
          fi
        done
        [ "$stopped" -gt 0 ] || echo "nothing to stop"
        # Every failure path above was an echo, and the final command evaluated
        # to 0 either way — so a night where NOTHING could be stopped reported
        # success and the sessions ran on until morning.
        [ "$failed" -eq 0 ] || { echo "$failed shutdown step(s) failed" >&2; exit 1; }
      '';
    };
  };
  mkRoutineService = r: {
...
in
{
  imports = [
    ...
    # Shared with the alarm host; the /etc policies it needs are documented there.
    ...
    text = ''
      context.modules = [
          { name = libpipewire-module-raop-discover
            args = {
                # The KEF will not hold a PCM session (raop-discover's default):
                # RTSP SETUP returns 500, the speaker drops the control
                # connection, and module-raop-sink calls
                # pw_impl_module_schedule_destroy on rtsp_disconnected, so the
                # sink deletes itself with nothing logged. Measured: PCM lasted
                # ~60s idle, ALAC lasted 41 minutes idle. ALAC is strictly
                # better here and the Sonos takes it too.
                #
                # NOT A FULL FIX. The KEF's sink still disappears ~30s after a
                # stream ENDS, and discover does not bring it back — it only
                # creates sinks on an mDNS add event, and the speaker never
                # stops advertising. Recovery is a pipewire restart. Tried and
                # rejected: static sinks (removes the discovery that keeps the
                # Sonos alive), raop.transport = tcp (died 4s after playback),
